...
|---|---------|-----|-----------|--------|
| H1 | No relay admission control; **relay binds `0.0.0.0` → LAN-reachable** | **HIGH** | primary; reach upgraded by blind-verify | LAN-reach FIXED; admission handshake BUILT on branch (browser-test owed) |
...
- **Resolved (LAN-reach) 2026-06-09:** pre-fix bind demonstrated as `::` (all interfaces); fixed to loopback (`HOST` env, default `127.0.0.1`), real-process bind netstat-confirmed `127.0.0.1:8080`; 12+20 tests green. **This closes the LAN-reach only.**
- **Admission handshake BUILT 2026-06-09 (branch `feat/h1-admission-handshake`, NOT merged):** the relay can't enforce admission without learning the passphrase (which would break relay-operator-proof), so admission is enforced **peer-to-peer** instead. On data-channel open each side sends an encrypted `__sdz-hello`; a peer is admitted only once it sends a frame we can **decrypt** (proving it holds the room key), and no app content is sent to an unadmitted peer. A peer that fails to prove the key within `AUTH_TIMEOUT_MS` (8s) is dropped from the mesh. This makes the **passphrase the admission boundary** — a wrong/absent-passphrase peer gets one hello ciphertext then is kicked, instead of silently receiving all traffic. In convenience mode it's moot (the id-derived key is shared), as expected. **Changes the live WebRTC path → cannot be Node-verified; on a branch with static-contract checks + a `BROWSER-TEST.md` plan, owed your two-tab test before merge.** Residual still OPEN: a relay-enforced join token (would need a shared secret with the relay) and the same-host-process case in convenience mode.
